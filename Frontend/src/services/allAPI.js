...
export const editLeaveStatusConductor = async (conductor_id, reqbody) => {
    return await commonAPI("PUT", `${SERVERURL}/editLeaveStatusConductor/${conductor_id}`, reqbody, '');
}
// Get All Conductor
...
export const getVechileByIdVD = async (vehicle_id) => {
    return await commonAPI("GET", `${SERVERURL}/getVehichleById/${vehicle_id}`, '', '');
}


export const EditVechicleStatus = async (vehicle_id, reBody) => {
    return await commonAPI("PUT", `${SERVERURL}/editVehicleById/${vehicle_id}`, reBody, '');
}